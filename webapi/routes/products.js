...
router.post("/",  upload.single('img'), async (req, res, next) => {
  try {
    ...
    const product = await productController.insert({name, price, description, category });
    return res.status(200).json(product);
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
...
// Cập nhật sản phẩm theo id
  router.put("/:id", upload.single('img'), async (req, res, next) => {
    try {
      ...
      const product = await productController.updateById(id, { name, price, description, categoryId, img });
      return res.status(200).json(product);
    } catch (error) {
